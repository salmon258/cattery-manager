 ...
 * payment proof on a reimbursement request.
 */
...
/**
 * POST — admin uploads a payment proof for a reimbursement request and
 * marks it paid in the same call (when ?mark_paid=1 is set or status='paid'
 * supplied as form field). Returns the updated row.
 *
 * Form: file (required), payment_date, payment_method, payment_reference
 */
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  if (markPaid && !paymentDate) {
    return NextResponse.json({ error: 'payment_date is required to mark paid' }, { status: 400 });
  }

  const admin = createServiceRoleClient();
  const ext = (file.name.split('.').pop() || 'bin').toLowerCase().slice(0, 10);
  const path = `reimbursements/${params.id}/payment-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 8)}.${ext}`;
