...
import type { Medication, MedRoute } from '@/lib/supabase/aliases';
...
export function EditMedicationModal({
  ...
  const m = useMutation({
    mutationFn: async (v: MedicationInput) => {
      if (!medication) throw new Error('No medication');
      const r = await fetch(`/api/medications/${medication.id}`, {
        method: 'PATCH',
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['medications-all', catId] });
  ...
  return (
    <ResponsiveModal open={open} onOpenChange={(o) => !o && onClose()} title={t('editTitle')}>
