...
interface Props {
  ...
  onClose: () => void;
  catIds: string[];
  /** Called after a successful batch insert (e.g. to exit select mode). */
  onSuccess?: () => void;
}

/**
 * Admin tool: create the same medication schedule for many cats at once.
 * Mirrors NewMedicationModal field-for-field — only the submission target
 * and the title differ — so validation and defaults stay in sync.
 */
export function BatchMedicationModal({ open, onClose, catIds, onSuccess }: Props) {
  ...
  const defaultValues: MedicationInput = {
    medicine_name: '',
    dose: '',
    route: 'oral',
    start_date: today,
  ...
  // Reset the form each time the modal is reopened so a previously submitted
  // draft does not leak into the next batch.
  useEffect(() => {
  ...
  const m = useMutation({
    mutationFn: async (v: MedicationInput) => {
      const r = await fetch('/api/medications/batch', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ cat_ids: catIds, medication: v })
      });
      if (!r.ok) throw new Error((await r.json()).error ?? 'Failed');
      return (await r.json()).created as number;
    },
    onSuccess: (created) => {
      toast.success(t('batchCreated', { count: created }));
      // Invalidate per-cat medication caches plus the shared task queues.
      catIds.forEach((id) => qc.invalidateQueries({ queryKey: ['medications', id] }));
  ...
  return (
    <ResponsiveModal
      ...
      title={t('batchScheduleTitle', { count: catIds.length })}
      description={t('batchScheduleDesc')}
    >
      <form onSubmit={form.handleSubmit((v) => m.mutate(v))} className="space-y-3 py-2">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" disabled={m.isPending || catIds.length === 0}>
