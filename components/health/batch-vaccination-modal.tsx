...
/**
 * Admin tool: record the same vaccination entry for many cats at once.
 * Mirrors LogVaccinationModal field-for-field — only the submission target
 * and the title differ — so validation and defaults stay in sync.
 */
export function BatchVaccinationModal({ open, onClose, catIds, onSuccess }: Props) {
  const t = useTranslations('vaccines');
  const tc = useTranslations('common');
  const qc = useQueryClient();

  const today = new Date().toISOString().slice(0, 10);
  const defaultValues: VaccinationInput = {
    vaccine_type: 'f3',
    vaccine_name: '',
  ...
  const m = useMutation({
    mutationFn: async (v: VaccinationInput) => {
      const r = await fetch('/api/vaccinations/batch', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ cat_ids: catIds, vaccination: v })
      });
    ...
    onSuccess: (created) => {
      ...
      catIds.forEach((id) => qc.invalidateQueries({ queryKey: ['vaccinations', id] }));
